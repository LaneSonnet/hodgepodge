...
public class SignUtil {
    ...
    public static String sign(Document xmlDocument, String privateKeyStr, String messageEle) throws XMLSecurityException, NoSuchAlgorithmException, InvalidKeySpecException, UnsupportedEncodingException {
        // xmlDocBytes待签名内容，整个报文
//        Document xmlDocument = getXmlDocument(xmlDocBytes);
        XMLSignature xmlSignature = new XMLSignature(xmlDocument, xmlDocument.getDocumentURI(), XMLSignature.ALGO_ID_SIGNATURE_RSA);
    ...
    public static boolean check(Document document, String publicKeyStr) throws NoSuchAlgorithmException, XMLSecurityException, InvalidKeySpecException {
        XMLSignature signature = new XMLSignature((Element) document.getElementsByTagName("ds:Signature").item(0), document.getDocumentURI());
